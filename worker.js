  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (p === '/api/law-content' && m === 'GET') {
        ...
        const cacheKey = `lawhtml_${lawtype}_${id}`;
        const cached = await env.DB.prepare('SELECT data, cached_at FROM news_cache WHERE category=?').bind(cacheKey).first();
        ...
        const apiUrl = `https://www.law.go.kr/DRF/lawService.do?OC=${OC}&target=${lawtype}&ID=${encodeURIComponent(id)}&type=HTML&mobileYn=Y`;
